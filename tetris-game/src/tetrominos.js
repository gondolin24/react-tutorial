export const TETROMINOS = {
    ...
    J: {shape: [[0, 'J', 0], [0, 'J', 0], ['J', 'J', 0]], color: '33,95,223'},
    L: {shape: [[0, 'L', 0], [0, 'L', 0], [0, 'L', 'L']], color: '223,173,36'},
    O: {shape: [['O', 'O'], ['O', 'O']], color: '223,217,36'},
    S: {shape: [[0, 'S', 'S'], ['S', 'S', 0], [0, 0, 0]], color: '48,211,56'},
    T: {shape: [[0, 0, 0], ['T', 'T', 'T'], [0, 'T', 0]], color: '100,61,198'},
    Z: {shape: [['Z', 'Z', 0], [0, 'Z', 'Z'], [0, 0, 0]], color: '227,78,78'}

};
export const randomTetrominos = () => {
    const tentrominos = 'IJLOSTZ';
    const ranT = tentrominos[Math.floor(Math.random() * tentrominos.length)];

    return TETROMINOS[ranT];
}
